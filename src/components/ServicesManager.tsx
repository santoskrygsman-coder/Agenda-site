        ...
        {!isAdding && (
          <button 
            onClick={() => {
              setIsAdding(true);
              setEditingId(null);
              setEditForm({ name: "", description: "", price: "", duration: "", requiresDeposit: false });
            }}
      ...
      {isAdding && (
        <div className="bg-[#FCFAFA] border border-[#F3E8E8] p-5 rounded-2xl mb-6 shadow-[0_4px_20px_rgba(0,0,0,0.02)]">
          ...
          <div className="space-y-4">
            ...
            <div className="pt-2">
              <label className="flex items-center gap-3 cursor-pointer p-4 rounded-xl border border-[#F3E8E8] bg-white hover:border-[#D9A0A0] transition-colors">
                <input 
                  type="checkbox" 
                  checked={editForm.requiresDeposit} 
                  onChange={e => setEditForm({...editForm, requiresDeposit: e.target.checked})} 
                  className="w-5 h-5 rounded text-[#A76D74] focus:ring-[#A76D74] border-[#D9A0A0] cursor-pointer"
                />
                <div>
                  <span className="font-bold text-[#3A3335] block text-sm">Exigir Sinal (Pagamento Prévio)</span>
                  <span className="text-xs text-[#8B7E7F]">O cliente será avisado no agendamento para pagar o sinal.</span>
                </div>
              </label>
            </div>
          ...
          <div key={service.id} className={`p-5 border ${service.active ? 'border-[#F3E8E8]' : 'border-[#F3E8E8] opacity-60'} rounded-2xl bg-white shadow-[0_4px_20px_rgba(0,0,0,0.02)] transition-all hover:border-[#D9A0A0]`}>
            {editingId === service.id ? (
              <div className="space-y-4">
                ...
                <div className="pt-2">
                  <label className="flex items-center gap-3 cursor-pointer p-4 rounded-xl border border-[#F3E8E8] bg-[#FCFAFA] hover:border-[#D9A0A0] transition-colors">
                    <input 
                      type="checkbox" 
                      checked={editForm.requiresDeposit} 
                      onChange={e => setEditForm({...editForm, requiresDeposit: e.target.checked})} 
                  ...
                  <div className="flex-1 pr-4">
                    ...
                    <div className="flex flex-wrap gap-2 mt-2 items-center">
                      ...
                      {service.requiresDeposit && (
                        <span className="text-xs font-bold tracking-wider text-[#D4A373] bg-[#FFF9F2] px-2.5 py-1 rounded-full">SINAL</span>
                      )}
                      {!service.active && (
